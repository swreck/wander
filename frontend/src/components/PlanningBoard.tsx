  ...
  return (
    <DraggableDayItem expId={exp.id}>
      <div className={`flex items-center gap-1.5 ${desktop ? "group" : ""}`}>
        ...
        >
          {exp.name}
          {exp.sheetRowRef && <span className="ml-0.5 text-[#b8a990]" title="Synced with Larisa's Japan Guide">↔</span>}
        </span>
        <button
          ...
          return (
            <DraggablePoolItem key={exp.id} expId={exp.id}>
              <div className="flex items-start gap-2.5 px-3 py-2.5 rounded-lg bg-white border border-[#f0ece5]
                ...
                >
                  <div className="text-sm text-[#3a3128] font-medium">{exp.name}{exp.sheetRowRef && <span className="ml-0.5 text-[#b8a990] text-xs font-normal" title="Synced with Larisa's Japan Guide">↔</span>}</div>
                  {rating != null && (
